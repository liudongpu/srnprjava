
<@m_mb_common_header nav="main"  button="back"/>


	<div class="mobile_lay_main">
		<div class="mobile_mlogin_body">

			<div class="mobile_mlogin_box">
				<div class="mobile_w_h30 "></div>
				<div class="mobile_w_h30 mobile_w_border_b"></div>
				<div class="mobile_mlogin_list mobile_w_border_b">

					<div class="mobile_mlogin_item mobile_w_border_b">
						<div class="mobile_mlogin_icon_base mobile_mlogin_icon_mail"></div>
						<div class="mobile_mlogin_input">
							<input type="text"  id="login_loginname"  placeholder="请输入您的邮箱" />
						</div>
					</div>

					<div class="mobile_mlogin_item">
						<div class="mobile_mlogin_icon_base mobile_mlogin_icon_pass"></div>
						<div class="mobile_mlogin_input">
							<input type="password"  id="login_loginpass" placeholder="请输入您的密码" />
						</div>
					</div>
				</div>
				<div class="mobile_w_h30 "></div>
				<div class="mobile_w_h20 "></div>
				<div class="mobile_base_buttom" onclick="zen.mobile.login_login()">登录</div>
				<div class="mobile_w_h30 "></div>
				<div class="mobile_mlogin_att">
					<div class="mobile_mlogin_att_left">
						<a href="/newsinfo/mreg-reg">免费注册</a>
					</div>
					<div class="mobile_mlogin_att_right">
						<a href="/newsinfo/mforget-forget">忘记密码</a>
					</div>
				</div>
			</div>
		</div>
	</div>

<@m_mb_common_footer />
</body>